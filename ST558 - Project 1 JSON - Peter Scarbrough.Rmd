...
Dimensinos | 2742 rows, 23 columns  
...
Determine how player ratings change as a function of genre, price, and publisher. Whether a game is single or multiplayer will also be considered. In particular there is interest in constructing some multivariate plots to look at possible interactions bewteen player ratings and the indicated variables. Since there are many categories of publisher and genre, this preliminary analysis will focus on a simplified approach that restricts analysis only the top 5 gaming genres and top 15 game publishers.

## Preparing the data

As a first step in preparing the data, I will remove all variables not of interest. This will keep player rating, price and publisher variables as well as the nested variable that contains information of which genre the game belongs to. 
...
# (v) print results: note genre column now converted into logical indicator columns
...
datatable(myData, options(scrollY="400px", scrollX="400px"))
```
...
Single variable and contingency tables will be used to asses the counts for each level of the categorical variables in the data.
...
* The breakdown by multi-player and genre shows an interesting feature where adventure games appear to have a noticeable drop-off compared to other categories. Since adventure games tend to be thematically single-player this could reflect the injection of an incompatible design choice into an incompatible genre.  
...
Some ideas for further investigation could be to include video game date of publish, since the quality of video game publishers may change considerably over time. GOG.com sales retro as well as modern games, which may have radically different flavors of consumer reception. It may also be interesting to dig deeper into the genre and publisher associations with player ratings to see if publishers tend to do well in all genres they produce, or if they tend to perform particularly well in just one or two genres. Exploratory data analysis suggests that publishers some publishers may tend to specialize and therefore it is speculated that much of the between-publisher differences in player rating may occur in their areas of specialization. Adding all publishers from the original data set may also provide further insights and help resolve other potential interactions.